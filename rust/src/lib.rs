...
///   when done using it.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn zcashlc_list_accounts(wallet_db_handle: *mut WalletDbHandle) -> *mut ffi::Accounts {
    let wallet_db_handle = AssertUnwindSafe(wallet_db_handle);
    let res = catch_panic(|| {
...
/// - `db_handle` must not be passed to two FFI calls at the same time.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn zcashlc_update_chain_tip(wallet_db_handle: *mut WalletDbHandle, height: i32) -> bool {
    let wallet_db_handle = AssertUnwindSafe(wallet_db_handle);
    let res = catch_panic(|| {
        let wallet_db_handle = unsafe { wallet_db_handle.as_mut() }
            .ok_or_else(|| anyhow!("Database handle is required"))?;
...
/// - `db_handle` must not be passed to two FFI calls at the same time.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn zcashlc_fully_scanned_height(wallet_db_handle: *mut WalletDbHandle) -> i64 {
    let wallet_db_handle = AssertUnwindSafe(wallet_db_handle);
    let res = catch_panic(|| {
        let wallet_db_handle = unsafe { wallet_db_handle.as_ref() }
            .ok_or_else(|| anyhow!("Database handle is required"))?;
...
pub unsafe extern "C" fn zcashlc_scan_blocks(
    ...
    let res = catch_panic(|| {
        ...
        match scan_cached_blocks(&network, fs_block_db_handle.db(), wallet_db_handle.db_mut(), from_height, &from_state, limit) {
...
pub unsafe extern "C" fn zcashlc_put_utxo(
    ...
    let res = catch_panic(|| {
        ...
        match wallet_db_handle.db_mut().put_received_transparent_utxo(&output) {
            Ok(_) => Ok(true),
...
/// - `fs_block_db_handle` must not be passed to two FFI calls at the same time.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn zcashlc_init_block_metadata_db(fs_block_db_handle: *mut FsBlockDbHandle) -> bool {
    let fs_block_db_handle = AssertUnwindSafe(fs_block_db_handle);
    let res = catch_panic(|| {
...
pub unsafe extern "C" fn zcashlc_rewind_fs_block_cache_to_height(
    ...
    let res = catch_panic(|| {
        ...
        fs_block_db_handle
            ...
            .map(|_| true)
            .map_err(|e| anyhow!("Error while rewinding block cache to height {}: {}", height, e))
    });
    unwrap_exc_or(res, false)
}
...
/// - `fs_block_db_handle` must not be passed to two FFI calls at the same time.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn zcashlc_latest_cached_block_height(fs_block_db_handle: *mut FsBlockDbHandle) -> i32 {
    let fs_block_db_handle = AssertUnwindSafe(fs_block_db_handle);
    let res = catch_panic(|| {
        let fs_block_db_handle = unsafe { fs_block_db_handle.as_ref() }
            .ok_or_else(|| anyhow!("Block database handle is required"))?;
    ...
    let res = catch_panic(|| {
        ...
        lwd_conn.with_taddress_transactions(
            ...
            |tx_data, mined_height| {
                ...
                decrypt_and_store_transaction(&network, wallet_db_handle.db_mut(), &tx, mined_height)?;
    ...
    let res = catch_panic(|| {
            ...
            .get_transparent_address_metadata(account_uuid, &addr)?
        {
            lwd_conn.with_taddress_utxos(
                ...
                |output| {
                    found = Some(addr);
                    wallet_db_handle.db_mut().put_received_transparent_utxo(&output)?;
                    Ok(())
                },
            )?;
        }
...
pub unsafe extern "C" fn zcashlc_tor_lwd_conn_check_single_use_taddr(
    ...
    let res = catch_panic(|| {
        ...
        let addrs = wallet_db_handle
            .db()
            .get_ephemeral_transparent_receivers(account_uuid, max_exposure_depth, true)?;
        ...
        if let Some((addr, meta)) = selected_addr_meta {
            ...
            if found.is_none() {
                ...
                wallet_db_handle.db_mut().schedule_next_check(&addr, offset_seconds)?;
            }
        }
